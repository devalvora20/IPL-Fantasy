    ...
    try {
        ...
        while (await stats.hasNext()) {
            ...
            let temp = { 
                ...
                "totalBattingPoints": player["total_batting_points"] ,
                "totalFieldingPoints": player["total_fielding_points"] ,
                "totalBowlingPoints": player["total_bowling_points"]
            };
            if(player["name"]=="Kohli"){
                break1++;
                if(break1==2){
                    break;
                }
...
app.get('/get-all-vice-captains', async (req, res) => {
    let viceCaptains = []

    try {
        const vcs = await getAllVcs();
        while (await vcs.hasNext()) {
            const owner = await vcs.next();
            viceCaptains.push({ "vc_points":owner["vc_points_per_match"],"name": owner["name"], "vc": owner["vc"] });
        }
        res.send({ "viceCaptains": viceCaptains });
    } catch (error) {
        console.log(error);
    }
});

app.post('/set-vice-captain',async function (req, res) {
    let body = req.body;
    try {
        const vc = await getVc(body["owner"]);
        console.log(vc);
    } catch (error) {
        console.log(error);
    }
    setVc(body["owner"], body["gw"], body["player"]);
    res.send({"success":"true"});
    
});

async function setVc(name, gw, player) {
    try {
        ...
        let key1 = "vc."+gw;
        let updateQuery = {}
        updateQuery[key1] = player;
        all_player_vcs = await dbo.collection("players").updateOne({"name":name},{"$set":updateQuery})
    }
    catch (e) {
        console.log(e);
    }
...
async function getVc(name) {
    let resp;
    try {
        client = new MongoClient(uri)
        const db = await client.connect();
        const dbo = db.db("ipl_fantasy");
        resp = await dbo.collection("players").findOne({"name":name})
    }
    catch (e) {
        console.log(e);
    }
...
async function getAllVcs() {
    ...
    try {
        ...
        const db = await client.connect();
        const dbo = db.db("ipl_fantasy");
        all_player_vcs = await dbo.collection("players").find({})
    }
    catch (e) {
        console.log(e);
    }
